...
function get_level1_categories($conn) {
    $stmt = $conn->query("SELECT id, name, description FROM categories WHERE level = 1 ORDER BY name");
    $categories = [];
    while ($row = $stmt->fetch()) {
        $row['id'] = (int)$row['id'];
        $categories[] = $row;
 ...
 * Returns an associative array keyed by parent name.
 *
 * @param PDO $conn  Database connection
 * @return array     [parent_name => [child_category, ...], ...]
 */
function get_categories_grouped($conn) {
    ...
    $grouped = [];
    while ($row = $stmt->fetch()) {
        $parent_name = $row['parent_name'] ?? 'Uncategorized';
        if (!isset($grouped[$parent_name])) {
            $grouped[$parent_name] = [];
        }
        $grouped[$parent_name][] = [
            'id' => (int)$row['id'],
